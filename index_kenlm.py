...
def process(lines, centers, lms):
    ...
    for fname, year, line_idx, line in lines:
        ...
        ppl = lm.perplexity(' '.join(line.replace(' ', 'ʖ')))
        output.append(
            {'fname': fname,
             'year': year,
             'lineNum': line_idx,
...
if __name__ == '__main__':
    ...
    for f in os.listdir(args.models_dir):
        start, stop = re.search('([0-9]{4})_([0-9]{4}).arpa', f).groups()
        lms[int(start), int(stop)] = kenlm.Model(os.path.join(args.models_dir, f))
    print("Done!\n")

    centers = {start + ((stop-start)/2): (start, stop) for start, stop in lms.keys()}

    fieldnames = ['fname', 'year', 'lineNum', 'perplexity', 'words']
